...
void aimbot::select_targets()
{
	...
	std::ranges::sort( targets, [] ( const target_data_t& a, const target_data_t& b )
	{
		if ( !a.player || !b.player )
			return a.player != nullptr;

		return a.player->get_origin().Dist( local_player->get_origin() ) < b.player->get_origin().Dist( local_player->get_origin() );
	} );
}
...
void aimbot::get_best_target( final_target_t& final_target )
{
	...
	static auto calc_target = [config] ( target_data_t* target ) -> std::vector<final_target_t>
	{
		...
		// Skeet-inspired latest+oldest strategy
		std::vector<lag_record_t*> records_to_scan{};

		// Always scan latest
		if ( !scan_records.empty() )
		{
			records_to_scan.push_back( scan_records.front() );
		}

		// Also scan oldest if different from latest and valid
		if ( scan_records.size() > 1 )
		{
			auto oldest = scan_records.back();
			const auto player = target->player;

			if ( oldest != scan_records.front()
				&& !oldest->m_dormant
				&& !oldest->m_ignore
				&& player->get_alive() )
			{
				records_to_scan.push_back( oldest );
			}
		}

		// Scan the selected records
		for ( auto it = records_to_scan.begin(); it != records_to_scan.end(); ++it )
		{
			auto& record = **it;
			...
			const auto health = target->player->get_predicted_health();
			if ( point.safety >= penetration::safety_no_roll && point.damage >= health )
				break;
		}
		...
		{
			aimpoint_t point = {};
			if ( !hitscan::get_best_hitpoint( target, *scan_records.front(), point, nullptr, local_player->get_eye_pos() + prediction::unpred_vel * interfaces::globals()->interval_per_tick * 4.f ) )
				return out;
...
//#endif
			...
			if ( !hitscan::get_best_hitpoint( target, log.extrapolated_record, point, extra ) )
			{
				...
				{
					point = {};
					if ( hitscan::get_best_hitpoint( target, log.extrapolated_record, point, extra, local_player->get_eye_pos() + prediction::unpred_vel * interfaces::globals()->interval_per_tick * 4.f ) )
	...
	{
		if ( aimbot_helpers::is_better_target( target, final_target, last_target ) )
			final_target = target;
	}
}
...
void aimbot::get_scan_records( C_CSPlayer* const player, std::vector<lag_record_t*>& ret, const bool ignore_extra )
{
	...
	lag_record_t* oldest = nullptr;
	lag_record_t* distant = nullptr;
	lag_record_t* angle_diff = nullptr;
	lag_record_t* defensive = nullptr;

	auto& log = player_log::get_log( player->EntIndex() );

	auto max_dist = 0.f;
	auto max_diff = 0.f;
	auto max_lby_diff = 0.f;
	...
	for ( auto i = log.record.size() - 1; i >= 0; i-- )
	{
		...
		if ( record.m_lagamt > sv_maxusrcmdprocessticks || !record.m_valid )
			continue;
		...
		if ( diff > 4.f && diff > max_diff )
		{
			max_diff = diff;
			add_shot( angle_diff );
		}

		const auto lby_diff = fabsf( math::normalize_float( record.m_eye_angles.y - record.m_lby ) );
		if ( lby_diff > 35.f && lby_diff > max_lby_diff )
		{
			max_lby_diff = lby_diff;
			add_shot( defensive );
		}
	}
	...
	if ( first && first->breaking_lc() )
		return;

	if ( shot && first != shot && !ignore_extra )
		ret.emplace_back( shot );

	if ( oldest && first != oldest && !ignore_extra )
		ret.emplace_back( oldest );
	...
	if ( defensive && first != defensive && defensive != angle_diff && !vars::aim.low_perf_scan_accuracy->get<bool>() )
		ret.emplace_back( defensive );
