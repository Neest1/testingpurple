...
static int get_shots_to_kill( const aimbot::final_target_t& target )
{
	const auto weapon = local_weapon;
	const auto is_singleshot = weapon && ( weapon->get_weapon_id() == WEAPON_SSG08 || weapon->get_weapon_id() == WEAPON_AWP || weapon->get_weapon_id() == WEAPON_REVOLVER || misc::retract_peek );
	const auto doubletap_ready = tickbase::fast_fire && weapon && !is_singleshot && tickbase::compute_current_limit() > 2;

	const auto health = target.player->get_predicted_health() + ( target.point.penetration_points.size() > 1 ? 5 : 0 );
	return static_cast< int >( ceilf( health / fminf( target.point.damage * ( doubletap_ready ? 2.f : 1.f ), static_cast< float >( health ) ) ) );
}

bool aimbot_helpers::is_better_target( const aimbot::final_target_t& candidate, const aimbot::final_target_t& current, const int last_target_index )
{
	if ( candidate.point.damage < 1.f )
		return false;

	if ( !current.record )
		return true;

	if ( current.predictive && !candidate.predictive )
		return true;

	if ( candidate.predictive && !current.predictive )
		return false;

	if ( candidate.record && current.record && candidate.record->m_index == last_target_index && globals::nospread )
		return false;

	const auto shots_to_kill = get_shots_to_kill( candidate );
	const auto shots_to_kill_current = get_shots_to_kill( current );

	if ( candidate.point.safety >= penetration::safety_no_roll && ( current.point.safety < penetration::safety_no_roll || shots_to_kill < shots_to_kill_current ) )
		return true;

	const auto same_safety_tier = ( candidate.point.safety >= penetration::safety_no_roll && current.point.safety >= penetration::safety_no_roll )
		|| ( candidate.point.safety < penetration::safety_no_roll && current.point.safety < penetration::safety_no_roll )
		|| candidate.point.safety == current.point.safety;

	if ( shots_to_kill_current == shots_to_kill && same_safety_tier )
	{
		if ( candidate.record && current.record && candidate.record->m_server_tick > current.record->m_server_tick )
			return true;

		if ( candidate.point.safety > current.point.safety )
			return true;

		if ( candidate.point.damage > current.point.damage )
			return true;

		if ( candidate.record && current.record && candidate.record->m_index == last_target_index )
			return true;
	}

	return shots_to_kill < shots_to_kill_current;
}
...
namespace
{
	bool use_base_style_autostop()
	{
		const auto weapon = local_weapon;
		if ( !weapon )
			return false;

		const auto cfg = config::current_config( weapon );
		if ( !cfg )
			return false;

		return cfg->predictive_autostop->get<bool>() || cfg->autostop_inbetween->get<bool>();
	}
}
...
void aimbot_helpers::slide_stop( CUserCmd* cmd )
{
	...
	if ( vars::misc.in_peek_assist() )
	{
		stop_to_speed( 0.f, cmd );
		return;
	}

	if ( use_base_style_autostop() )
	{
		const auto wpn_data = weapon->get_wpn_data();
		if ( !wpn_data )
			return;

		const auto scoped = local_player->get_scoped();
		const auto max_speed = scoped ? wpn_data->flmaxplayerspeedalt : wpn_data->flmaxplayerspeed;
		stop_to_speed( max_speed * 0.25f, cmd );
		return;
	}

	const auto is_revolver = weapon->get_weapon_id() == WEAPON_REVOLVER;
	const auto max_speed = is_revolver ? 180.f : weapon->get_max_speed();
	const auto wpn_data = weapon->get_wpn_data();
	const auto scoped = local_player->get_scoped() && wpn_data;
	const auto accuracy_speed = 0.33f * ( scoped ? wpn_data->flmaxplayerspeedalt : max_speed );

	prediction::run_prediction( cmd->command_number - 1 );
	const auto velocity = prediction::get_pred_info( cmd->command_number - 1 ).velocity;
	const auto speed2d = velocity.Length2D();

	if ( speed2d <= accuracy_speed + 1.f )
	{
		const auto squirt = sqrtf( ( cmd->forwardmove * cmd->forwardmove ) + ( cmd->sidemove * cmd->sidemove ) );
		if ( squirt > 0.01f )
		{
			cmd->forwardmove = cmd->forwardmove / squirt * accuracy_speed;
			cmd->sidemove = cmd->sidemove / squirt * accuracy_speed;
		}
		else
		{
		...
		stop_to_speed( accuracy_speed, cmd );
		return;
	}

	QAngle angle{};
	math::vector_angles( velocity * -1.f, angle );
	angle.y = math::normalize_float( cmd->viewangles.y - angle.y );

	Vector direction{};
	math::angle_vectors( angle, &direction );

	cmd->forwardmove = direction.x * speed2d;
	cmd->sidemove = direction.y * speed2d;
}
