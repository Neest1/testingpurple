...
bool hitscan::get_best_hitpoint( aimbot::target_data_t* target, lag_record_t& record, aimbot::aimpoint_t& ret, lag_record_t* extra, Vector eyepos )
{
	...
	// freestand/sideways peek: resolver matrices diverge, strict intersection safepoints often empty
	if ( !bot && record.m_last_ang_differs && safety > penetration::safety_no_roll )
		safety = penetration::safety_no_roll;

	// jitter / defensive AA: prefer hitting over strict safepoint when angles are unreliable
	if ( !bot && ( record.m_pitch_jitter || record.m_last_ang_differs ) && safety == penetration::safety_full )
		safety = penetration::safety_no_roll;

	const auto try_scan = [&] ( const int try_safety ) -> aimbot::aimpoint_t
	{
		auto result = hitscan_record( target, record, try_safety, extra, eyepos );
		return result.damage >= 1 ? result : aimbot::aimpoint_t{};
	};
	...
	if ( safety > penetration::safety_no_roll )
	{
		ret = try_scan( penetration::safety_no_roll );
		if ( ret.damage >= 1 )
			return true;
	}
	...
	{
		ret = try_scan( penetration::safety_none );
		if ( ret.damage >= 1 )
			return true;
	}
...
void hitscan::setup_skeet_hitbox_points( circular_buffer<aimbot::aimpoint_t, 78>& points, int index, const studiohdr_t* studio_hdr, lag_record_t& record, const Vector& eyepos, float spread_radius, int safety, CCSWeaponData* weapon_data, const weaponconfig_t* config, int minimum_damage, bool multipoint_enabled )
{
	...
	const auto bones = record.matrix( resolve_dir );

	float scale = config->pointscale->get<float>() * 0.01f;
	...
	Vector world_center{};
	{
		const auto local_center = ( hitbox->bbmin + hitbox->bbmax ) * 0.5f;
		math::vector_transform( local_center, bones[ hitbox->bone ], world_center );
	}

	if ( hitbox->radius > 0.f )
	{
		const auto radius = std::max( hitbox->radius - spread_radius, 0.f );
		scale = bscale = clamp( radius / hitbox->radius, 0.f, 1.f );
	}

	const auto point_safety = is_bot ? penetration::safety_full : safety;
	std::vector<Vector> local_points{};

	if ( hitbox->radius <= 0.f )
	{
		matrix3x4_t rot_matrix{};
		math::angle_matrix( hitbox->rotation, rot_matrix );

		matrix3x4_t matrix = math::concat_transforms( bones[ hitbox->bone ], rot_matrix );

		const auto origin = Vector( matrix[ 0 ][ 3 ], matrix[ 1 ][ 3 ], matrix[ 2 ][ 3 ] );
		...
		if ( index == HITBOX_RIGHT_FOOT || index == HITBOX_LEFT_FOOT )
		{
			...
			local_points.emplace_back( center.x, center.y, center.z + d1 );

			if ( multipoint_enabled )
			{
				const auto d2 = ( hitbox->bbmin.x - center.x ) * scale;
				const auto d3 = ( hitbox->bbmax.x - center.x ) * scale;
				local_points.emplace_back( center.x + d2, center.y, center.z );
				local_points.emplace_back( center.x + d3, center.y, center.z );
			}
		}

		if ( local_points.empty() )
			local_points.emplace_back( center );

		for ( auto& p : local_points )
		{
			Vector rotated{};
			math::vector_rotate( p, matrix, rotated );
			p = rotated + origin;
		}
	}
	else
	{
		const auto r = hitbox->radius * scale;
		const auto br = hitbox->radius * bscale;
		const auto center = ( hitbox->bbmin + hitbox->bbmax ) * 0.5f;
		...
		if ( index == HITBOX_HEAD )
		{
			...
			if ( multipoint_enabled )
			{
				constexpr auto rotation = 0.70710678f;
				local_points.emplace_back( hitbox->bbmax.x + ( rotation * r ), hitbox->bbmax.y + ( -rotation * r ), hitbox->bbmax.z );
				local_points.emplace_back( hitbox->bbmax.x, hitbox->bbmax.y, hitbox->bbmax.z + r );
				local_points.emplace_back( hitbox->bbmax.x, hitbox->bbmax.y, hitbox->bbmax.z - r );
				local_points.emplace_back( hitbox->bbmax.x, hitbox->bbmax.y - r, hitbox->bbmax.z );

				if ( record.m_velocity.Length() <= 0.1f && record.m_eye_angles.x <= 0.f )
					local_points.emplace_back( hitbox->bbmax.x - r, hitbox->bbmax.y, hitbox->bbmax.z );
			}
		}
		else if ( index == HITBOX_BODY )
		{
			local_points.emplace_back( center );
			if ( multipoint_enabled )
				local_points.emplace_back( center.x, hitbox->bbmax.y - br, center.z );
		}
		...
		else if ( index == HITBOX_RIGHT_CALF || index == HITBOX_LEFT_CALF )
		{
			local_points.emplace_back( center );
			if ( multipoint_enabled )
				local_points.emplace_back( hitbox->bbmax.x - ( hitbox->radius / 2.f ), hitbox->bbmax.y, hitbox->bbmax.z );
		}
		...
		for ( auto& p : local_points )
			math::vector_transform( p, bones[ hitbox->bone ], p );
	}

	for ( const auto& p : local_points )
	{
		const auto point = points.emplace_back( p, world_center, index, hitbox->group );
		point->no_pen_damage = minimum_damage;
		point->center_dist = p.DistToSqr( world_center );
		point->safety = point_safety;
		point->safety_size = is_bot ? 10.f : 0.f;
	}
}
...
void hitscan::multipoint_hitbox( circular_buffer<aimbot::aimpoint_t, 78>& points, int index, const studiohdr_t* studio_hdr, lag_record_t& record, const Vector& eyepos, float spread_radius, bool is_knife, bool is_zeus, const weaponconfig_t* config, int safety, lag_record_t* extra, CCSWeaponData* weapon_data )
{
	...
	if ( hitbox->radius <= 0.f )
	{
		setup_skeet_hitbox_points( points, index, studio_hdr, record, eyepos, spread_radius, safety, weapon_data, config, minimum_damage, pointscale_hitboxes[ index ] );
		return;
	}
	...
	if ( index == HITBOX_HEAD && !( record.m_flags & FL_ONGROUND ) && player->get_move_type() != MOVETYPE_LADDER )
		rs *= 0.7f;

	if ( !ConVar::cl_lagcompensation || !ConVar::cl_predict )
		rs *= 0.8f;

	const auto spread_scale = index == HITBOX_BODY || index == HITBOX_PELVIS || index == HITBOX_UPPER_CHEST ? 0.05f : 0.1f;
	rs -= spread_radius * spread_scale;
	...
	for ( auto i = 0; i < 3; i++ )
	{
		...
		for ( auto j = resolver_direction::resolver_networked; j < resolver_direction::resolver_direction_max; j++ )
		{
			...
			if ( !( j == resolve_dir && !is_extra_record
				...
				|| j == resolver_direction::resolver_zero ) )
				continue;
	...
	const auto add_resolved_fallback = [&] ( int fallback_safety ) -> void
	{
		const auto out_fb = record.matrix( resolve_dir )[ hitbox->bone ];
		Vector vmin_fb{};
		Vector vmax_fb{};
		math::vector_transform( hitbox->bbmin, out_fb, vmin_fb );
		math::vector_transform( hitbox->bbmax, out_fb, vmax_fb );
		const auto center_fb = ( vmin_fb + vmax_fb ) * 0.5f;

		const auto effective_rs = pointscale_hitboxes[ index ] ? rs : 0.f;

		const auto point = points.emplace_back( center_fb, center_fb, index, hitbox->group );
		point->no_pen_damage = minimum_damage;
		point->center_dist = 0.f;
		point->safety = is_bot ? penetration::safety_full : fallback_safety;
		point->safety_size = is_bot ? 10.f : 0.f;

		if ( effective_rs < 0.2f )
			return;

		const auto right = forward.Cross( Vector( 0, 0, 1 ) ) * hitbox->radius * effective_rs;
		const auto top = Vector( 0, 0, 1 ) * hitbox->radius * effective_rs;
		const auto bottom = Vector( 0, 0, -1 ) * hitbox->radius * effective_rs;

		for ( const auto& offset : { right, right * -1.f, top, bottom } )
		{
			const auto p = center_fb + offset;
			const auto edge = points.emplace_back( p, center_fb, index, hitbox->group );
			edge->no_pen_damage = minimum_damage;
			edge->center_dist = p.DistToSqr( center_fb );
			edge->safety = is_bot ? penetration::safety_full : fallback_safety;
			edge->safety_size = is_bot ? 10.f : 0.f;
		}
	};
	...
	if ( safety == penetration::safety_none && ( index != HITBOX_HEAD || !config::current_config( local_weapon )->baim.high_risk->get<bool>() || is_bot || vars::aim.headshot_only() ) )
	{
		...
		if ( !extract_points( intersect, penetration::safety_none ) )
			add_resolved_fallback( penetration::safety_none );
	}
	...
	if ( safety != penetration::safety_full && !extract_points( intersect, penetration::safety_no_roll ) )
		add_resolved_fallback( penetration::safety_no_roll );

	intersect = poly_intersect::get_intersection_poly( intersect, mapped_points[ 0 ][ static_cast< int >( resolver_direction::resolver_zero ) ] );

	if ( record.m_last_ang_differs )
		intersect = poly_intersect::get_intersection_poly( intersect, mapped_points[ 1 ][ static_cast< int >( resolver_direction::resolver_zero ) ] );

	if ( use_extra )
		intersect = poly_intersect::get_intersection_poly( intersect, mapped_points[ 2 ][ static_cast< int >( resolver_direction::resolver_zero ) ] );

	if ( !extract_points( intersect, penetration::safety_full ) )
		add_resolved_fallback( penetration::safety_no_roll );
...
bool hitscan::check_enemy_hittable( C_CSPlayer* enemy )
{
	...
	for ( const auto& record : records )
	{
		...
		const auto& dir = log.get_dir( record );
		const auto model = enemy->get_model();
		if ( !model )
			continue;
		...
		for ( const auto hb : { HITBOX_BODY, HITBOX_CHEST, HITBOX_UPPER_CHEST, HITBOX_PELVIS } )
		{
			const auto hitbox = studio_hdr->pHitbox( hb, 0 );
			if ( !hitbox )
				continue;

			const auto bones = record->matrix( dir );
			Vector vmin{}, vmax{};
			math::vector_transform( hitbox->bbmin, bones[ hitbox->bone ], vmin );
			math::vector_transform( hitbox->bbmax, bones[ hitbox->bone ], vmax );
			const auto center = ( vmin + vmax ) * 0.5f;

			if ( penetration::check_hittable( { index, eye_pos, center } ) )
			{
				hittable = true;
				break;
			}
	...
	if ( !hittable && !records.empty() && config::current_config( weapon )->predictive_autostop->get<bool>() && !vars::aim.fake_duck->get<bool>() && prediction::unpred_vel.Length2D() > 20.f )
	{
		...
		eye_pos = local_player->get_eye_pos() + prediction::unpred_vel * interfaces::globals()->interval_per_tick * 4.f;
		const auto target_position = Vector( record->m_origin.x, record->m_origin.y, record->m_origin.z + 75.f );
		const auto target_angle = math::calc_angle( eye_pos, target_position );
...
void hitscan::calculate_hitbox_data( hitbox_data_t& out, C_CSPlayer* player, int hitbox_idx, matrix3x4_t* matrix )
{
	const auto model = player->get_model();
	const auto studio_hdr = interfaces::model_info()->GetStudioModel( model );
	const auto hitbox = studio_hdr->pHitbox( hitbox_idx, 0 );

	if ( !hitbox )
			return;

	const auto bone_matrix = matrix[ hitbox->bone ];

	out.hitbox_id = hitbox_idx;
	out.bone = hitbox->bone;
	out.radius = hitbox->radius;
	out.is_obb = hitbox->radius == -1.f;

	math::vector_transform( hitbox->bbmin, bone_matrix, out.mins );
	math::vector_transform( hitbox->bbmax, bone_matrix, out.maxs );
}

bool hitscan::is_safe_point( const Vector& eye_pos, const Vector& aim_point, C_CSPlayer* player, lag_record_t& record, lag_record_t* extra )
{
	if ( !player || !player->get_model() )
			return false;

	const auto studio_hdr = interfaces::model_info()->GetStudioModel( player->get_model() );
	if ( !studio_hdr )
			return false;

	Vector forward = ( aim_point - eye_pos ).Normalized();

	int hits = 0;

	// Helper lambda to check a single matrix
	auto check_matrix = [&] ( matrix3x4_t* matrix ) -> bool
	{
			if ( !matrix )
				return false;
			...
			for ( int i = 0; i < HITBOX_MAX; i++ )
			{
				...
				hitbox_data_t hb_data = {};
				calculate_hitbox_data( hb_data, player, i, matrix );

				bool intersects = false;

				if ( hb_data.is_obb )
				{
					// OBB hitbox - use inverse transforms
					Vector local_eye, local_aim;
					math::vector_itransform( eye_pos, matrix[ hitbox->bone ], local_eye );
					math::vector_itransform( aim_point, matrix[ hitbox->bone ], local_aim );

					intersects = math::intersect_bb( local_eye, local_aim, hb_data.mins, hb_data.maxs );
				}
				else
				{
					// Capsule hitbox - simple ray-capsule test
					Vector dir = aim_point - eye_pos;
					Ray_t ray;
					ray.Init( eye_pos, dir );
					intersects = intersection::does_intersect_capsule( ray, hb_data.mins, hb_data.maxs, hb_data.radius );
				}
			...
			return matrix_hits > 0;
	};

	// Check against 3 matrices (left, right, center bones)
	if ( check_matrix( record.matrix( resolver_direction::resolver_networked ) ) )
			hits++;

	if ( record.extra_matrix( record.m_setup_dir ) && check_matrix( record.extra_matrix( record.m_setup_dir ) ) )
			hits++;

	if ( extra && extra->matrix( record.m_setup_dir ) && check_matrix( extra->matrix( record.m_setup_dir ) ) )
			hits++;
