...
bool lagcomp::is_record_in_window( const lag_record_t* record, float sim_time, float window )
{
	if ( !record )
		return false;

	const auto net_channel = interfaces::engine()->GetNetChannelInfo();
	if ( !net_channel )
		return false;

	// Compute latency correction (outgoing + incoming lerp)
	float outgoing_latency = net_channel->GetLatency( FLOW_OUTGOING );
	float incoming_latency = net_channel->GetLatency( FLOW_INCOMING );
	float lerp_time = lagcomp::get_lerp_time();

	var( sv_maxunlag );
	float correction = outgoing_latency + incoming_latency + lerp_time;
	correction = clamp( correction, 0.f, sv_maxunlag->get_float() );

	// Check if record's simtime falls within the window
	float time_diff = fabsf( sim_time - record->m_simtime + correction );
	return time_diff <= window;
}

void lagcomp::build_backtrack_records( int player_index, float window, std::vector<lag_record_t*>& out )
{
	auto& log = player_log::get_log( player_index );
	...
	for ( auto i = 0; i < log.record.size(); i++ )
	{
		...
		if ( record->m_ignore || record->m_dormant )
			continue;

		if ( is_record_in_window( record, interfaces::globals()->curtime, window ) )
		{
...
void lagcomp::filter_extended_backtrack( const std::vector<lag_record_t*>& source, std::vector<lag_record_t*>& out )
{
	out.clear();
	...
	// Extended backtrack filtering:
	// Keep the newest record and a subset of older ones
	if ( !source.empty() )
	{
		out.emplace_back( source.front() );  // Keep newest

		if ( source.size() > 1 )
		{
			// Add records at intervals
			const int interval = static_cast< int >( source.size() / 3 );

			if ( interval > 1 )
			{
				for ( size_t i = interval; i < source.size(); i += interval )
				{
					out.emplace_back( source[ i ] );
				}
			}

			out.emplace_back( source.back() );  // Keep oldest
		}
	}
}
