...
namespace
{
	resolver_direction get_brute_direction( player_log_t& log, int desync_side );
	...
	void refresh_low_delta_state( player_log_t& log, const lag_record_t* record, const float foot_yaw )
	{
		...
		const auto left_low = math::normalize_float( record->m_lby - 35.f );
		const auto right_low = math::normalize_float( record->m_lby + 35.f );
	...
	resolver_direction select_resolver_direction( player_log_t& log, C_CSPlayer* player, lag_record_t* record, const lag_record_t* previous, const int freestand_side, const int desync_side, const float foot_yaw )
	{
		auto& side_data = log.m_mode[ log.m_current_mode ].m_side[ log.m_current_side ];

		enum_array<resolver_direction, float, resolver_direction::resolver_direction_max> scores = {};
		scores.fill( 0.f );

		const auto speed = record->m_velocity.Length2D();
		const auto moving = speed > 4.f;
		const auto fast = speed > 90.f;
		const auto crouching = record->m_duckamt > .55f;
		const auto sideways = is_sideways( player, record );
		const auto jitter = is_jittering( log );
		const auto defensive = is_defensive_tick( record, previous );
		const auto fake_flick = is_fake_flick( record, previous );
		const auto low_delta = is_low_delta_record( record, foot_yaw );
		const auto static_aa = is_static_antiaim( log );
		const auto brute_dir = get_brute_direction( log, desync_side != 0 ? desync_side : 1 );

		auto add_score = [&scores] ( const resolver_direction direction, const float score )
		{
			scores[ direction ] += score;
		};

		add_score( side_data.m_current_dir, 2.f );
		add_score( resolver_direction::resolver_networked, 1.f );

		if ( record->m_did_wall_detect )
			add_score( side_data.m_current_dir, 12.f );

		if ( freestand_side != 0 && sideways )
		{
			const auto freestand_dir = side_to_direction( freestand_side );
			add_score( freestand_dir, 13.f );
			add_score( opposite_direction( freestand_dir ), 3.f );
		}
		else if ( desync_side != 0 )
			add_score( side_to_direction( desync_side ), moving ? 4.f : 7.f );

		if ( moving )
		{
			add_score( resolver_direction::resolver_networked, fast ? 18.f : 11.f );
			if ( sideways || crouching )
				add_score( brute_dir, 5.f );
		}
		else
		{
			add_score( brute_dir, 8.f );
			add_score( resolver_direction::resolver_zero, 5.f );
		}

		if ( crouching )
		{
			add_score( brute_dir, 7.f );
			add_score( resolver_direction::resolver_zero, 7.f );
		}

		if ( low_delta )
		{
			add_score( resolver_direction::resolver_zero, 10.f );
			if ( log.m_stored_brute_yaw != 0.f )
				add_score( direction_from_yaw( *record, log.m_stored_brute_yaw, record->m_eye_angles.y ), 9.f );
		}

		if ( static_aa && !moving )
			add_score( brute_dir, 9.f );

		if ( jitter )
		{
			add_score( resolver_direction::resolver_networked, 10.f );
			add_score( resolver_direction::resolver_zero, 4.f );
		}

		if ( defensive )
		{
			add_score( resolver_direction::resolver_networked, 14.f );
			add_score( resolver_direction::resolver_zero, 8.f );
			if ( crouching || sideways )
				add_score( brute_dir, 4.f );
		}

		if ( fake_flick )
		{
			add_score( resolver_direction::resolver_zero, 12.f );
			add_score( resolver_direction::resolver_networked, 7.f );
		}

		if ( !log.m_unknown )
			add_score( side_data.m_current_dir, 5.f );
		...
		{
			const auto direction = static_cast< resolver_direction >( i );
			auto score = scores[ direction ];

			if ( side_data.m_blacklist[ direction ] && direction != resolver_direction::resolver_networked )
				score -= 20.f;
	...
	{
		if ( !local_player || log.record.size() < 2 || !is_sideways( player, record ) )
			return 0;
		...
		return neg_trace.fraction < pos_trace.fraction ? -1 : 1;
	}

	resolver_direction get_brute_direction( player_log_t& log, const int desync_side )
	{
		// Bruteforce state machine - cycles through resolver_max, resolver_min, resolver_zero
		// Uses modulo 3 to cycle: 0 = max, 1 = min, 2 = zero
		switch ( log.m_shots % 3 )
		{
			case 1:
				return desync_side > 0 ? resolver_direction::resolver_min : resolver_direction::resolver_max;
			case 2:
				return resolver_direction::resolver_zero;
			default:
				return desync_side > 0 ? resolver_direction::resolver_max : resolver_direction::resolver_min;
		}
	}

	// Reset bruteforce state when player state changes significantly
	void reset_bruteforce_state( player_log_t& log )
	{
		...
		// Reset blacklist for all modes and sides
	...
	// Check if we should reset bruteforce state based on record changes
	bool should_reset_bruteforce( player_log_t& log, const lag_record_t* current, const lag_record_t* previous )
	{
		if ( !current )
			return false;
		
		// Reset if player is dormant (re-entering PVS)
		if ( current->m_dormant )
			return true;
		
		// Reset if velocity changed significantly (player started/stopped moving)
		if ( previous )
		{
...
void resolver::apply_desync_side( C_CSPlayer* player, lag_record_t* record, const lag_record_t* previous )
{
	...
	const auto moving = record->m_velocity.Length2D() > 4.f;
	const auto crouching = record->m_duckamt > .55f;
	const auto sideways = is_sideways( player, record );
	const auto defensive = is_defensive_tick( record, previous );
	const auto fake_flick = is_fake_flick( record, previous );
	...
	if ( record->m_did_wall_detect && !defensive && !fake_flick )
		return;

	if ( moving && !crouching && !sideways && !defensive && !fake_flick )
	{
		current_dir = resolver_direction::resolver_networked;
		return;
	}
	...
	if ( !log.m_unknown )
	{
		if ( defensive || fake_flick )
			current_dir = select_resolver_direction( log, player, record, previous, freestand_side, log.m_desync_side, foot_yaw );
...
void resolver::update_moving_side( lag_record_t* record, const lag_record_t* previous )
{
	...
	log.m_mode[ log.m_current_mode ].m_side[ log.m_current_side ].m_current_dir = get_brute_direction( log, log.m_desync_side );
}
...
void resolver::calc_missed_shots( shot_t* shot )
{
	...
	if ( shot->hit && player && player->get_alive() )
	{
		...
		// Increment shots to cycle to next brute direction
		log->m_shots++;
		
		// Update the resolver direction for next shot based on new m_shots value
		const auto current_mode = shot->record.m_resolver_mode;
		const auto current_side = shot->record.m_resolver_side;
		const auto desync_side = log->m_desync_side;
		
		// Update the brute direction for the next shot
		if ( desync_side != 0 )
		{
			log->m_mode[ current_mode ].m_side[ current_side ].m_current_dir = get_brute_direction( *log, desync_side );
		}
	}
