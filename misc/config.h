...
struct weaponconfig_t
{
	...
	inline int get_mindmg( C_CSPlayer* player = nullptr, int highest_damage = 0 ) const
	{
		...
		const auto minimum_dmg = clamp( static_cast< int >( mindmg_override_enabled->get< bool >() ? mindmg_override->get< float >() : mindmg->get< float >() ), 0, 125 );
		if ( !player )
			return minimum_dmg;
		...
		return player->get_predicted_health() + ( minimum_dmg - 100 );
	}
	value* hitchance;
	value* pointscale;
	value* damage_accuracy;
